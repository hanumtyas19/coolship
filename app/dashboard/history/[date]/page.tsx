...
export default function HistoryDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchDetailData = async () => {
      try {
        console.log("📅 Fetching data untuk tanggal:", date);
        
        const logsRef = collection(firestore, `tracking_logs/${date}/logs`);
        const logsSnapshot = await getDocs(logsRef);
        
        console.log("🕐 Jumlah time slots:", logsSnapshot.size);
        
        if (logsSnapshot.empty) {
          setError("Tidak ada data log untuk tanggal ini");
          return;
        }
    ...
    return (
      <div className="p-6 max-w-6xl mx-auto">
        <div className="text-center py-12">
          ...
          <p className="text-gray-600">Memuat data detail...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <h1 className="text-3xl font-bold text-gray-900">Detail Perjalanan</h1>
          <p className="text-gray-600 mt-1">Tanggal: {date}</p>
        </div>
      </div>
      ...
      {error ? (
                ...
                <div key={slot.time} className="p-6">
                  ...
                  {slot.data.length > 0 ? (
                    ...
                    <div className="text-center text-gray-500">
                      Tidak ada data untuk waktu ini
                    </div>
                  )}
                </div>
              ))}
        ...
        <div className="bg-gray-50 rounded-lg p-12 text-center">
          ...
          <p className="text-gray-500">Tidak ada log perjalanan untuk tanggal {date}</p>
        </div>
      )}
    </div>
  );
